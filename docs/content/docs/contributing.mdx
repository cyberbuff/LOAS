---
title: Contributing
description: "Learn how to contribute to LOAS by writing YAML test files"
icon: FileCode
---

## Writing YAML Test Files

This repository uses YAML files to define tests that can be executed on macOS systems. Each YAML file represents a specific MITRE ATT&CK technique and contains multiple atomic tests.
...
tests:
  - name: "Test Name"
    ...
    cleanup:
      - description: "How to undo state created by the test"
        command: |
          # Cleanup AppleScript or JavaScript code here
    args:
...
- **`tcc_required`** (array, optional): TCC services required by the test, such as `"Automation"`, `"Accessibility"`, `"Screen Recording"`, or `"Microphone"`. Defaults to `[]`
- **`elevation_required`** (boolean, optional): Whether the test requires elevated privileges
- **`safety`** (object, optional): Safety metadata. `destructive` and `modifies_system` both default to `false`
- **`cleanup`** (array, optional): Cleanup steps with `description` and `command` fields
...
tests:
  ...
  - name: Get user information (using System Events JXA)
    ...
    tcc_required: ["Automation", "Accessibility"]
    command: |-
...
4. **Proper Permissions**: Set `tcc_required` services and `elevation_required` flags appropriately
